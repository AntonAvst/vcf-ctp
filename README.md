...
**Steps in order:**
- **0. Collar candidate selector** (new): given `session.start_dt` / `session.end_dt`, scan the collar data drive and filter down to only AnimalIds that show meaningful activity (ΔKineticsCountR above threshold) during the video window. Produces a small candidate set — cows not in the pen have near-zero kinetic activity and are automatically excluded. Feeds this filtered set into the kinetic matcher. No manual roster needed (activity filter — Option B).
