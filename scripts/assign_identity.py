...
Usage:
    python3 assign_identity.py \\
        ...
        --kinetics  ~/thesis_workspace/raw_data/CollarData/kinetic_data_6366_7507_7513.csv \\
    ...
    python3 assign_identity.py \\
        --db      ... --session ... --assign 2:7507 1:6366 \\
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--kinetics", default="",
                    help="kinetic_data_*.csv — required when --run_reconcile is set")
    ap.add_argument("--gallery_dir",      default="./reid_gallery")
    ap.add_argument("--embed_parquet",    default="",
                    help="Path to embeds.parquet (auto-detected from DB dir if omitted)")
...
def run_reconcile(args: argparse.Namespace, db_path: str) -> None:
    section("Running reconcile.py")

    if not args.kinetics:
        raise ValueError(
            "--kinetics is required when --run_reconcile is set.\n"
            "  Add: --kinetics ~/thesis_workspace/raw_data/CollarData/kinetic_data_*.csv"
        )
    if not Path(args.kinetics).exists():
        raise FileNotFoundError(f"Kinetics file not found: {args.kinetics}")

    # auto-detect embed_parquet from DB directory if not provided
    embed_parquet = args.embed_parquet
    if not embed_parquet:
        candidate = Path(db_path).parent / "embeds.parquet"
        if candidate.exists():
            embed_parquet = str(candidate)
            log(f"Auto-detected embed_parquet: {embed_parquet}")
        else:
            log("No embeds.parquet found alongside DB — cosine gallery will be skipped.")
...
#
# DB=~/thesis_workspace/outputs/tracks/refet33_2024-12-21/calving_project.db
# KIN=~/thesis_workspace/raw_data/CollarData/kinetic_data_6558_7509_7774.csv
# GAL=~/thesis_workspace/reid_gallery
#
...
# python3 assign_identity.py --db $DB --session refet33_20241221 --list
#
# # Remove a wrong assignment and redo:
# python3 assign_identity.py --db $DB --session refet33_20241221 --remove 71
# python3 assign_identity.py --db $DB --session refet33_20241221 \
#   --kinetics $KIN --gallery_dir $GAL \
